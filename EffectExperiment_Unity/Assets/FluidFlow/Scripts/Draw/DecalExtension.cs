...
namespace FluidFlow
{
    public static class DecalExtension
    {
        ...
        public static void ProjectDecal(this FFCanvas canvas, FFDecal decal, FFProjector projector, bool fadeBasedOnSurfaceAngle = true, bool paintBackfacing = false)
        {
            ...
            command.SetViewProjectionMatrices(projector.View, projector.ProjectionGPU());
    ...
    public struct FFProjector
    {
        ...
        public Matrix4x4 ProjectionGPU()
        {
        ...
        {
            return new FFProjector(flipZ * transform.worldToLocalMatrix,
                                   Matrix4x4.Ortho(-width * .5f, width * .5f, -height * .5f, height * .5f, near, far));
        }

        public static FFProjector Orthogonal(Ray ray, Vector3 up, float width, float height, float near, float far)
        {
            return new FFProjector(flipZ * Matrix4x4.TRS(ray.origin, Quaternion.LookRotation(ray.direction, up), Vector3.one).inverse,
                                   Matrix4x4.Ortho(-width * .5f, width * .5f, -height * .5f, height * .5f, near, far));
        }
        ...
        {
            return new FFProjector(flipZ * transform.worldToLocalMatrix, Matrix4x4.Perspective(fov, aspect, near, far));
        }
